package io.github.thecsdev.extendedworldselection.client.gui.temp_port;
...
public final class TStackTracePanel extends TRefreshablePanelElement
{
	...
	// --------------------------------------------------
	private final MouseDragHelper dragHelper = new MouseDragHelper() {
		protected final @Override void apply(int dX, int dY) { move(dX, dY); }
	};
	// ==================================================
					...
					Arrays.stream(ExceptionUtils.getStackTrace(throwable).split("\\r?\\n"))
						.forEach(stackTraceElement ->
						{
							...
							final var s_lines = Arrays.stream(stackTraceElement.split("\\r?\\n"))
									...
									.toArray(Text[]::new);
							UILayout_initLines(panel_st, color, s_lines);
						});
				}
	...
	@Internal
	@Experimental
	private static final void UILayout_initLines(TPanelElement panel, int color, Text...texts)
	{
